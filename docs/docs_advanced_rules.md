...
## 12. 物理環境に合わせたトピック作成ポリシー

- 背景: ローカルや検証環境ではブローカー数が少なく（例: 1台構成）、設定上の `ReplicationFactor` と物理環境のブローカー数が不一致で `Replication factor: N larger than available brokers: M.` により作成が失敗することがある。
- 方針: 物理環境のブローカー数を優先し、作成時の RF を安全側（最小1）に自動調整する。
  - RF がブローカー数を超えていた場合、まずブローカー数に合わせて切り下げを試みる。
  - クラスタメタデータの取得に失敗した場合や、それでも作成に失敗した場合は RF=1 で 1 回リトライする。
  - 既存トピックがある場合は更新は行わずスキップ（CreateTopics での更新は不可）。
- 実装: KafkaAdminService（CreateDbTopicAsync / EnsureTopicExistsAsync）に環境検知と再試行を実装。
- 影響範囲: ローカル/単一ブローカー構成での「RF過大」に起因するテスト失敗や起動失敗を回避。
